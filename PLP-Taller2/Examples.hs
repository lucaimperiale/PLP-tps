...
--expr 38 = "zip 0 :: [] and (\\z -> z) :: [] with x,y ~> succ(x)"
expr 38 = "zip 0 :: [] and (\\z -> z) :: [] with x,y ~> succ(y)"
expr 39 = "zip x :: y :: [] and 0 :: [] with x,y ~> succ(y)"
expr 40 = "zip (\\f -> 0) :: [] and y with x,y ~> succ(y)"
--expr 42 = "zip (\\x -> x) :: [] and [] with x,y ~> g x"
--expr 43 = "zip 0 :: [] and (\\y -> y) :: [] with x,z ~> y z"
expr 41 = "zip n :: [] and 0 :: n with x,y ~> succ(x)"
...
--sol 38 = "{} >> zip (0 :: []{Nat}) and ((\z:t0 -> z) :: []{t0 -> t0}) with x,y ~> succ(x) : [Nat]"
sol 38 = "Cannot unify t2 -> t2 and Nat"
sol 39 = "{x:t0, y:t0} >> zip (x :: (y :: []{t0})) and (0 :: []{Nat}) with x,y ~> succ(y) : [Nat]"
sol 40 = "{y:[Nat]} >> zip ((\f:t0 -> 0) :: []{t0 -> Nat}) and y with x,y ~> succ(y) : [Nat]"
--sol 42 = "{g:(t0 -> t0) -> t1} >> zip ((\x:t0 -> x) :: []{t0 -> t0}) and []{t2} with x,y ~> g x : [t1]"
--sol 43 = "{y:(t0 -> t0) -> t1} >> zip (0 :: []{Nat}) and ((\y:t0 -> y) :: []{t0 -> t0}) with x,z ~> y z : [t1]"
sol 41 = "Cannot unify Nat and [Nat]"
